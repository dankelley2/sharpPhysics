...
public class PrefabDesignerGame : IGame
{
    ...
    private void CreateToolbar()
    {
        ...
        // Calculate starting X to center the toolbar (9 buttons now)
        int numButtons = 9;
    ...
    public void Update(float deltaTime, KeyState keyState)
    {
        ...
        _currentMousePos = keyState.MousePosition;
        ...
        if (keyState.EnterPressed && _currentMode == DrawMode.Polygon && _currentPolygonPoints.Count >= 3)
        {
            FinishPolygon();
            return;
        }
        ...
        if (keyState.Escape) return; // Already handled above
        ...
        if (keyState.LeftMousePressed)
        {
            ...
            if (keyState.MouseScreenPosition.Y < TOOLBAR_HEIGHT)
            {
                _uiManager.HandleClick(keyState.MouseScreenPosition);
                return;
            }
        ...
        if (!keyState.LeftMouseDown && _isDrawing &&
        ...
        if (keyState.LeftMouseDown && _drawStartPoint.HasValue)
        {
            _isDrawing = true;
        }
    }
    ...
    public void Render(Renderer renderer)
    {
        ...
        string modeText = _currentMode switch
        {
            ...
            _ => "Select a tool from the toolbar (P/C/R for shapes, W/A for constraints)"
        };
